...
public class AddExpenseDialog {
    ...
    /** Returns [tourId (Long), expense (Expense)] or null if cancelled. */
    public static Object[] show(List<Tour> tours) {
        ...
        dialog.setResultConverter(btn -> {
            ...
            if (tourBox.getValue() == null || categoryBox.getValue() == null
                    || categoryBox.getValue().trim().isEmpty() || amountField.getText().trim().isEmpty()) {
                Toast.error("Tour, Category and Amount are required.");
                return null;
            }
            float amount;
            try {
